...
# --- CONFIGURACIÓN ---
# Estos datos se leen de la sección "Secrets" de Streamlit Cloud
...
# --- BLOQUE 1: IDENTIFICACIÓN (INSTANTÁNEO) ---
...
if doc:
    # Validación de formato para Persona o Empresa
    if re.match(r"^[0-9XYZ][0-9]{7}[A-Z]$", doc):
        ...
        st.error("Formato de documento no válido. Revisa el NIF/CIF.")

# --- BLOQUE 2 Y 3: DIRECCIÓN Y VEHÍCULO (DENTRO DEL FORMULARIO) ---
# Solo se muestra si el DNI es válido y el usuario ha empezado a rellenar el nombre/razón social
if (tipo_sujeto == "PERSONA" and nombre) or (tipo_sujeto == "EMPRESA" and razon_social):
    ...
    with st.form("resto_formulario"):
        ...
        if enviado:
            # Validación de campos obligatorios antes de generar el XML
            if not calle or not num_dir or not municipio or not cp or not matricula or not marca or not modelo:
                st.error("Por favor, rellena todos los campos de dirección y vehículo.")
            elif len(cp) != 5 or not cp.isdigit():
                st.error("El Código Postal debe tener 5 números.")
            else:
                # Construcción del XML con valores fijos (MOTIVO: OTROS, DOCUMENTOS: CAMBIO TITULAR)
                ...
                if enviar_email(f"justificante_{matricula}.ga.xml", xml_content):
                    st.success("✅ ¡Archivo generado y enviado correctamente a la Gestoría!")
